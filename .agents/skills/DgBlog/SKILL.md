...
||---------|-----------------|---------------|
|| Create new blog post | [DgNotes Create Workflow](DgNotes/SKILL.md#create-workflow) (content-type: blog) | "write a blog about X" |
|| Edit or improve post | [DgNotes Improve Workflow](DgNotes/SKILL.md#improve-workflow) | "improve blog post X" |
|| Translate post to French | [Translation Workflow](#translation-workflow) | "translate X to French" |
|| Publish draft blog post | [DgNotes Publish Workflow](DgNotes/SKILL.md#publish-workflow) | "publish blog post X" |
|| Sync post to GitHub | [DgQuartzDev Deployment Workflow](DgQuartzDev/SKILL.md#deployment-workflow) | "sync the garden" |
|| Validate blog structure | Add `--validate` to any workflow | Use on create/improve |
...
Invoked when the user says "translate X to French", "traduis X en français", or "create FR version of Y".

### Quick Reference
- **File location**: `content/blog/fr/<kebab-case-french-title>.md`
- **Frontmatter additions**: `language: "fr"`, `source: "<original-filename>"`
- **Source post update**: Add `fr_translation: "fr/<french-filename>"` to original

### Execution Steps

1. **Load Rules**: Read `TranslationWorkflow.md` for complete EN→FR conventions
2. **Read Source**: Read the English blog post in full
3. **Target Path Derivation**: Determine French filename using kebab-case French title
...
5. **Content Translation**: Translate body text using Soushi888 voice conventions
   - Analytical but accessible: maintain technical precision in French
   - No emdashes (critical rule)
...
6. **Cross-Reference Update**: Add `fr_translation: "fr/<french-filename>"` to source English frontmatter
7. **Review**: Validate frontmatter completeness, check for emdashes, verify voice consistency
8. **Memory Update**: Append new translation patterns to `dg-patterns.md`
...
| English | French |
|---------|--------|
| `[[rust|Rust]]` | `[[rust|Rust]]` (keep technical targets, translate display text) |
